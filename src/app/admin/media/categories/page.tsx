...
const getColumns = (
  onEdit: (item: MediaCategoryDTO) => void,
  onDelete: (id: number) => void
): ColumnDef<MediaCategoryDTO>[] => [
  {
    accessorKey: 'id',
    header: 'ID Kategori',
    cell: ({ row }) => (
  ...
  const handleSave = async (name: string) => {
    try {
      ...
      if (selectedCategory?.id) {
        await editMediaCategory(selectedCategory.id, name);
        toast.success('Kategori berhasil diperbarui');
      } else {
        await addMediaCategory(name);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <GenericCategoryDialog
        ...
        description="Berikan nama kategori yang mudah dimengerti oleh pengguna."
        label="Nama Kategori Media"
        colorTheme="emerald"
      />
    </div>
  );
